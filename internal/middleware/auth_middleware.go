...
func NewRefreshMiddleware(secretKey string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if !ok {
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{"error": "Invalid token type in token"})
		}
		if tokenType != "refresh" {
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{"error": "Invalid type of token"})
		}
